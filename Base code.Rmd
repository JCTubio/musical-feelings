...
## Data cleaning
The original data set contained 103 responses, but some had to be droped. 21 participants had not finished the whole questionnaire and were thus deleted. Furthermore, four participants took less than four minutes to complete the questionnaire. This was considered unreasonably low and these responses were deleted. After these cleaning process, the dataset contained 78 responses.  
```{r}
...
**H1: Perceived happiness can predict songs valence**

*Simple linear regression* In order to check this prediction, we will perform a simple linear regression in which we try to predict the valence of the song, given the participant's happiness rating. 
...
*Fitting the model and evaluating the results* The model seems to fit adequately well. Both the intercept and the slope coefficients are significant and the R2 = 0.1211. This can be interpreted as happiness score being able to explain 12% of the variance in the song's valence. 
...
*Visualizing the relationship between happiness and valence*
...
*Visualizing perceived happiness vs Spotify's valence level*

```{r}
reg_ds
data_by_song <- reg_ds %>%
  group_by(song)%>%
  summarise(MedianHappiness = median(hppy), valence = valence * 7)
#data_per_song = 
#reg_ds%>% 
#  ggplot(aes(mean(hppy), valence))+ 
#  geom_boxplot(fill = '#1DB954')+
#  xlab('Happiness rating')+
#  ylab('Song valence')+
#  labs(title = 'Songs rated as happier tend to have a higher valence',
...
**H2: The relationship between happiness and valence is moderated by music sophistication**

*Moderation analysis* To check this prediction we perform a moderation analysis. We keep the standardized version of the happiness scores because of the same reasons exposed in the simple linear regression.  
...
*Checking extra assumption* Performing a moderation analysis implies entering the domains of the multiple linear regression (i.e. now there os more than one predictor). This means that the multicollinearity between predictors must be checked. To do so we look at the VIF values of the two predictors used, and there seems to be no multicollinearity at all.
```{r}
library(car)
vif(fitMod)
...
*Fitting the model* The model points at the interaction term being not significant and R2 = 0.1043. This points at our prediction not being met: the interaction is not significant and including this interaction term into the regression 
...
ggplot(plot_ds,aes(x=factor(hppy),y=valence,fill = grp))+
  ...
  labs(title = 'Sophistcation has no moderation effect', subtitle = 'Valence distributions at each level of happiness by sophistication levels')
...
*Performing the Levene's test and plotting* As we can see from the corrected Levene's tests p-value, none of the songs groups have different variances. From visual inspection we can also see that the dark green box does not tend to be smaller than the other two.
...
*Considerations surounding these results* It could be the case that our sample has a low sophistication in general. This would mean that our 'high' group still has a low sophistication in absolute terms, and thus we are not able to infer anything about highly sophisticated people and variance in happiness scores. To check this we plot the music sophistication distribution in our sample. We can see here nevertheless, that sophistication levels are sufficiently spreaded and we can consider our sample representative enough. 
...
var_ds%>%
  ...
  labs(title = 'Sample shows spreaded enough sophistication scores',
